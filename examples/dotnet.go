...
func (t testpad) EncodePayload(RawPadOraclePayload []byte) (encodedPayload string) {
	...
	encodedPayload = strings.ReplaceAll(encodedPayload, "/", "_")
	fmt.Sprintf("%s%d", strings.ReplaceAll(encodedPayload, "=", ""), strings.Count(encodedPayload, "="))
...
func (t testpad) DecodeCiphertextPayload(EncodedPayload string) []byte {
	...
	EncodedPayload = strings.ReplaceAll(EncodedPayload, "-", "+")
	EncodedPayload = strings.ReplaceAll(EncodedPayload, "_", "/")
	count := string(EncodedPayload[len(EncodedPayload)-1])
	EncodedPayload = EncodedPayload[:len(EncodedPayload)-1]
	padz, err := strconv.Atoi(count)
	if err == nil {
		// last digit requires padding
		for i := 0; i < padz; i++ {
			EncodedPayload += "="
		}
	}
	decoded, err = base64.StdEncoding.DecodeString(EncodedPayload)
	fmt.Println(err, EncodedPayload)
